...
schemas it will never call: at ~80 tokens a schema (`progressive-tool-disclosure`), a
dozen tools is ~1k tokens on every classification, the cost a gate exists to avoid.
It also inherits the answerer's memory, so the classification returns next turn as if
...
capability set — **or when a number must be independently decomposable.** A second
prompt is not a second role; it becomes one under that same clause, the day it
arrives with its own capability set or its own cost line. Forty roles nobody reads
is the same failure from the other end. **A sub-agent is a role with its own assembly**,
never a reuse of the parent's assembled agent — that inherits exactly what this
section forbids merging (`subagent-context-isolation`: when one is worth its cost).
...
That a model is addressed by role at all is `llm-cost-observability`'s rule; what
composition adds is where the map lives and what a role may be called. **A role is
a job, not a model tier.** A role named `flash` satisfies the borrowed rule
completely and still breaks the day it changes vendor — the tag stays stable, the
thing it names does not, and a migration reads as a latency regression in `flash`.
Name a role for the work it decides, and use that one literal string in
configuration, metrics, logs and evals alike.

**One component owns `role → (provider, model id, temperature, timeout, output cap,
optional fallback provider + id)`**, reads it from configuration, and hands out a
client by role name. The fallback is a field on that tuple, not a sibling role — the
same job on another vendor — and written any other way it is the migration bug above,
on the path nothing exercises until the primary rate-limits, mid-incident. A model
name in application code puts that name everywhere a model is chosen — clients,
tests, fixtures, the eval harness — and one is missed the day you migrate, leaving
the summariser on the expensive model for months.
...
attached one layer up, inspecting nothing. Two ways a control still never runs:
...
**A provider resolved once cannot disclose.** Built at startup or hung at the
invocation layer it returns one fixed tool set — activation changes nothing the
model sees, and no test fails.
...
summariser's "never calls a tool" *and* the **or acts** half of the classifier's, a
role with no tools having nothing to act with; a matching hash is "never writes its own
prompt"; and the embedder's holds once the index records the model id that built it and
the snapshot asserts the query handle equals it — pin only the query side and a role
emits the expected handle while querying another model's index. What is left over is
the classifier's other half, **answers an in-scope turn**: behaviour, not wiring, so an
eval case (`agentic-evals`). The hash catches a prompt someone *edited*, in the diff
that caused it; §4's catches one that *varies at runtime*.
...
on roles that clear it. Or clear it on purpose: move per-turn material above the
conversation — few-shot examples, the scope description, a tool set you were
disclosing lazily — until the prefix passes. Those tokens bill once at the write
rate and ~0.1× per read after, so it pays where one prefix serves many turns and
loses outright for a role that sees about one turn per prefix.
...
unlucky user on a request indistinguishable from every other: a quantized in-process
embedding model, measured at **5.7 s** to load, lands at p99.9, reads as a network
blip, and is never diagnosed. Measure your own; it never becomes rare either — once
per deploy, then per replica, then on every autoscale event.

**The stronger reason is that construction is when configuration is validated.** A
missing credential or an unknown model name makes an eagerly built process refuse to
start, and the deploy rolls itself back; built lazily, each is a 500 on the first
request needing that role, days after the deploy that caused it if the role is rare.
Keep readiness red until the eager work finishes.
...
4. Look up the cacheable-prefix floor for each role's model; diff the standing prompt
   — two turns, then two processes, on the bytes sent — only for the roles that clear
   it, and for one that does not, decide whether material moves in on purpose.
