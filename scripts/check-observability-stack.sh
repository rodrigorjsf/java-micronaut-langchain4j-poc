...
#       Tempo stored the trace, and Grafana provisioned its data sources, its dashboard
#       AND its alert rules.
#
...
#       again — a malformed ALERT RULE is skipped exactly that way, leaving a healthy Grafana
#       watching nothing. Reading the metric names off a running collector, and the rules
#       back out of the provisioning API, is the only honest check.
#
...
echo "==> grafana alerting"
# Read BACK, never inferred from a healthy container. A provisioned alert rule with a bad
# field, an unknown datasource uid or a malformed `data` block is logged once at startup and
# then SKIPPED — Grafana comes up perfectly healthy with no rules at all, which looks exactly
# like a quiet system. The provisioning API is the only honest evidence they loaded.
RULES=$(curl -fsS http://localhost:3001/api/v1/provisioning/alert-rules || echo '[]')
RULE_COUNT=$(jq 'length' <<<"${RULES}" 2>/dev/null || echo 0)
if [[ "${RULE_COUNT}" -gt 0 ]]; then
  ok "${RULE_COUNT} alert rule(s) provisioned"
  jq -r '.[] | "        - " + .title' <<<"${RULES}" 2>/dev/null || true
else
  fail "no alert rules provisioned — check: docker logs agentic-grafana | grep -i provision"
fi

# A rule with no route notifies nothing, which is the same silence as having no rule.
CONTACTS=$(curl -fsS http://localhost:3001/api/v1/provisioning/contact-points || echo '[]')
if [[ "$(jq 'length' <<<"${CONTACTS}" 2>/dev/null || echo 0)" -gt 0 ]]; then
  ok "$(jq 'length' <<<"${CONTACTS}") contact point(s) provisioned"
else
  fail "no contact points — every alert would fire into nothing"
fi

# The tree is what hands a firing alert to a contact point. Grafana 13 ships an autogenerated
# tree whose every branch routes to a built-in receiver named `empty`, which has no
# integrations — so a stack with rules, a contact point and the DEFAULT tree notifies exactly
# nobody, with everything green. Provisioning the tree overwrites it; this reads back that the
# overwrite happened and landed on our receiver.
ROUTE=$(curl -fsS http://localhost:3001/api/v1/provisioning/policies || echo '{}')
RECEIVER=$(jq -r '.receiver // empty' <<<"${ROUTE}" 2>/dev/null)
if [[ -n "${RECEIVER}" && "${RECEIVER}" != "empty" ]]; then
  ok "the notification policy routes to '${RECEIVER}'"
else
  fail "the root policy routes to '${RECEIVER:-nothing}' — alerts would fire into nowhere"
fi
